...
Phone 1.0.70 (92), TV 1.0.70 (93), Wear 1.0.70 (94) all build with the existing Play upload key. Credentials remain outside Git. The upload certificate matches Play; the phone OAuth upload-key SHA-1 also matches google-services.json. The **Google Play app-signing** OAuth client is now registered and its persisted package/SHA-1 were verified on September 25. The app keeps the existing backend Web client ID; this server-side registration needs no new binary. Real Play-installed sign-in remains unverified.
...
- Google Cloud now contains both upload-certificate and Play app-signing Android OAuth clients. The new MegaRadio Google Play client was already created when inspected. Its saved package com.megaradio and production SHA-1 CA:38:14:B6:BC:19:B4:F1:EB:C4:C7:C9:C4:7E:BF:C1:E4:A8:CE:04 match Play. Audience is External / In production; there are no configured sensitive or restricted scopes. See `validation/google-oauth-signing.json`.
