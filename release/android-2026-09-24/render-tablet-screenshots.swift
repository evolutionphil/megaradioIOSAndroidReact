...
let frames = [("discover","01-discover.png"),("player","02-player.png"),("genres","03-genres.png"),("country","04-country.png"),
              ("search","05-search.png"),("rock-stations","06-rock-stations.png"),("sleep-timer","07-sleep-timer.png"),("favorites","08-favorites.png")]
let options = Array(CommandLine.arguments.dropFirst(4))
let only = options.first(where: { !$0.hasPrefix("--") }) ?? ""
let extrasOnly = options.contains("--extras-only")
...
for locale in captions.keys.sorted() where only.isEmpty || locale == only {
    ...
    for (index,frame) in frames.enumerated() {
        if extrasOnly && index < 4 { continue }
