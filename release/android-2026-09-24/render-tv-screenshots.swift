...
for locale in captions.keys.sorted() where only.isEmpty || locale == only {
    let folder=output.appendingPathComponent(locale).appendingPathComponent("tv")
    ...
    for (index,frame) in frames.enumerated() {
        ...
        let filename=String(format:"MegaRadio-%@-tv-%02d-%@.png",locale,index+1,frame.0)
        let file=folder.appendingPathComponent(filename)
        try rep.representation(using:.png,properties:[:])!.write(to:file)
        audit.append(["locale":locale,"kind":frame.0,"file":file.path,"source":raw.path,"headline":words[0],"caption":words[1],"headingFont":heading,"captionFont":subtitle,"width":1920,"height":1080,"capturedUILanguage":"English","editorialTextLocalized":true])
    }
}
try JSONSerialization.data(withJSONObject:audit,options:[.prettyPrinted,.sortedKeys]).write(to:output.appendingPathComponent(only.isEmpty ? "tv-render-audit.json" : "tv-render-preview-\(only).json"))
print("Rendered \(audit.count) actual Android TV screenshots")
