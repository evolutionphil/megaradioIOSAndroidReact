...
2026 at 13:29 Europe/Vienna. Apple later rejected its metadata under Guideline
2.3.6 because the Advertising age-rating descriptor was false. The descriptor is
now verified true in both editable App Information records. The same build was
resubmitted at 20:00 Europe/Vienna and is Waiting for Review, with automatic
release after approval. macOS 1.0.3 and tvOS 1.0.0 (1) passed processing and
...
## iPad artwork refresh

The requested four-image design uses genuine iPad captures of discovery, live
playback, a genre station grid and country selection. All 50 locale sets / 200
images passed local validation in
`/Users/mumiix/Downloads/MegaRadio-iPad-Redesign-2026-09-24/`.
The editable rejected version now permits delivery without withdrawing review.
`deliver-ipad-redesign.py --apply` targets only iOS 1.0.70 and replaces originals
only after all four new images in a set are processed and checksum verified.
The canonical image package and delivery evidence are updated on full success.
All 50 sets / 200 images are delivered and verified. See
`validation/ipad-redesign-delivery.json` for per-locale evidence.

`verify-ios-resubmission.py` reads the exact iOS build association, 150 metadata
records, automatic release setting and both Advertising descriptors. It does
not modify or submit anything. The final delivery gate also requires evidence
that Advertising is declared before allowing another review handoff.
...
Genres capture. The refreshed iPad set uses four real app screens with localized captions
and the original MegaRadio icon. Mac and Apple TV use
...
commit. Eight offline delivery tests cover acknowledged-but-uncommitted uploads, reuse, ordering, interruption, and
