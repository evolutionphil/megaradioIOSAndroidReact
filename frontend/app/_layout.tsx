...
export default function RootLayout() {
  ...
  // Play at Login - Execute after onboarding is done and user is authenticated
  useEffect(() => {
    const executePlayAtLogin = async () => {
      // Only execute once, after onboarding check is done
      if (hasExecutedPlayAtLogin || !hasCheckedOnboarding || showSplash) {
        return;
      }
      ...
      try {
        ...
        setHasExecutedPlayAtLogin(true);
        ...
        if (stationToPlay) {
          // Use playerStore to set the station - AudioProvider will handle playback
          const { setCurrentStation, setPlaybackState, setMiniPlayerVisible } = usePlayerStore.getState();
          setCurrentStation(stationToPlay);
          setMiniPlayerVisible(true);
          console.log('[PlayAtLogin] Station set for playback:', stationToPlay.name);
        }
      } catch (error) {
    ...
    executePlayAtLogin();
  }, [hasCheckedOnboarding, hasExecutedPlayAtLogin, showSplash, isAuthenticated, favorites]);
