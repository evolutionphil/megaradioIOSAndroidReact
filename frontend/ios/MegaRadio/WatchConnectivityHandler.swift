...
class WatchConnectivityHandler: NSObject {
    ...
    @objc func updatePlaybackState(_ isPlaying: Bool) {
        currentNowPlaying["isPlaying"] = isPlaying
        sendNowPlayingToWatch()
    ...
    private func sendMessageToWatch(_ message: [String: Any]) {
        latestContext.merge(message) { _, new in new }
        guard let session = session, session.activationState == .activated else { return }
        // Persist latest state even while reachable; newly launched watch reads it.
...
extension WatchConnectivityHandler: WCSessionDelegate {
    ...
    func sessionReachabilityDidChange(_ session: WCSession) {
        ...
        if session.isReachable {
            ...
            DispatchQueue.main.async { self.sendNowPlayingToWatch() }
        }
    }
    ...
    func session(_ session: WCSession, didReceiveMessage message: [String : Any], replyHandler: @escaping ([String : Any]) -> Void) {
        ...
        // Same normalized WatchStation payload for messages AND replies.
        DispatchQueue.main.async { replyHandler(self.latestContext) }
    }
